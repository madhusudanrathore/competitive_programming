...
int main(){
	unsigned int t, k, p1, p2, sum, quo;
	...
	while(t--){
		...
		else if( quo*k==sum ){ // sum is multiple of k
			if( quo%2==0 ){printf("CHEF\n");} // even
			else{printf("COOK\n");} // odd
		}
		else{ // sum is not multiple of k
			if( quo%2==0 ){printf("CHEF\n");} // previous multiple was even
			else{printf("COOK\n");} // previous multiple was odd
		}
	}
	return 0;
}